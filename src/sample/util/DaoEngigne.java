...
public class DaoEngigne {
    ...
    public static String constructDaoSaveRequette(Object obj) throws Exception {
        ...
        requette += ") VALUES(";
        List<String> list=lunchGetters(obj);
        list.remove(0);
        requette += assembleRequestItem(list, "'");
        requette += ")";
        return requette;
    }
    ...
    public static void generateTables(String beanPath) throws Exception {
        ...
        File file = new File("src/sample/" + beanPath);
        String[] files = file.list();
        for (int i = 0; i < files.length; i++) {
            String myFile = files[i];
            myFile = myFile.replace(".java", "");
            Class myClass = Class.forName("sample." + beanPath + "." + myFile);
                ConnectDB.executeUpdate(constructDaoCreateTableRequette(myClass));
        }
    ...
    private static String typeJavaToSql(String typeRetour) throws Exception {
        if (typeRetour.equals("String")) {
            ...
            return ("Date");
        }else if (typeRetour.equals("LocalDateTime")) {
            return ("datetime");
        }
        return "";
    }
